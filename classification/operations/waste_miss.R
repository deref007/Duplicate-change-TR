...
source("C://Users/yuanruifan/bitbucket/jit_defect_prediction/classification/packages/imbalance.R")
source("C://Users/yuanruifan/bitbucket/jit_defect_prediction/classification/packages/CBS.R")
source("C://Users/yuanruifan/bitbucket/jit_defect_prediction/classification/packages/VarImportance.R")
source("C://Users/yuanruifan/bitbucket/jit_defect_prediction/classification/packages/one-way.R")
...
# projects <- c("hbase", "geronimo")
projects <- c("tuscany", "hcommon")
...
calculated_measures <- c("auc", "precision", "recall", "f1measure", "gmean", "fp", "fn", "waste_effort", "all_effort", "waste_perc")
calculated_measures2 <- c("auc", "precision", "recall", "f1measure", "gmean", "fp", "fn", "waste_effort", "all_effort", "waste_perc")
...
for (p in projects){
	...
	for (szz_label in szz_labels){
		...
		baseline_form <- as.formula(paste(szz_baseline, var_names_str, sep = " ~ "))
		...
		for (i in 1:bootstrap_times){
			...
			all_inspected_lines <- append(all_inspected_lines, waste_miss_results[4])
			waste_perc <- append(waste_perc, waste_miss_results[5])
		}
		...
		if (method == "imbalance" | method == "balance"){
			...
			result_frame <- store_result_to_frame(result_frame, all_inspected_lines)
			result_frame <- store_result_to_frame(result_frame, waste_perc)
			
		}

		names(result_frame) <- calculated_measures2
		# print(szz_label)
		result_fn <- paste(c(root_path, "result_", method, "/", p, "_", classifier, "_", szz_label, ".csv"), collapse="")
