projects <- c(  "hcommon", "geronimo","hbase","pig")
projectnames <- c( "Hadoop C.", "Geronimo","HBase", "Pig")
...
root_path <-  "E://tmp//time_eware_data//result//new-exp/"

# auc, f1measure, 
#"waste_effort","all_effort"
compare_measures <- c("auc")

models <- append(models, rep(c("Old", "New"), 2 * length(compare_measures)))

first_column <- c("RF", rep("", 3), "LR", rep("", 3), "NB", rep("", 3))
...
for (classifier in classifiers){
	for (p in projects){
		print(p)
		fn <- paste(c(root_path, classifier,  "_",compare_label,"_",p, "_exp", ".csv"), collapse="")
		data <- read.csv(fn)
		...
		for(compare_measure in compare_measures){
			for (szz_label in szz_labels){
				temp_fn <- paste(c(root_path, classifier,  "_",szz_label,"_",p,  "_exp", ".csv"), collapse="")
...
geroot_path<-"E://tmp//time_eware_data//result//new-exp//result/"
save_path <-paste(c(root_path, "rq6-time_validation-", compare_measures, ".csv"), collapse="")
