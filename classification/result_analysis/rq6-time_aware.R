...
#root_path <- "E://tmp//time_eware_data//new_exp//result/"
###two-
root_path <- "E://tmp//time_eware_data//result//new-exp/"

classifiers <- c("RF", "LR", "NB")

classifier_col <- c("RF", rep("", 3), "LR", rep("", 3), "NB", rep("", 3))
...
for (classifier in classifiers){
	for (p in projects){
		...
		for (measure in measures){
			for (label in labels){
				result_fn <- paste(c(root_path,  classifier, "_",label, "_", p, "_exp",".csv"), collapse="")
...
save_path <-paste(c(ge_root_path, "rq7-balance-", measures, ".csv"), collapse="")
