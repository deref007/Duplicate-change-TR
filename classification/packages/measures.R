...
mcc<-function(test_data,prob,label){
	obs<-as.numric(test_data[label][,1]=="buggy")
	confunsion_matrix<-confusion.matrix(obs,prob)
	tp<-confusion_matrix[2,2]
...
# Using built-in methods in R, MUCH FASTER!!
Popt2 <- function(ordered_data, total_churn, var_la, var_ld, label, label_val){
	real_buggy_data <- ordered_data[which(ordered_data[label][,1] == label_val),]
	...
	buggy_label_vector <- as.numeric(ordered_data[label][,1] == label_val)
...
Popt <- function(test_data, ordered_data, label, WORST=FALSE){
	real_buggy_data <- test_data[which(test_data[label][,1] == "buggy"),]
	total_churn <- sum(test_data$real_la + test_data$real_ld)
	...
	remaining_buggy_data <- test_data[-which(test_data[label][,1] == "buggy"),]
	...
	for(i in 1:n_instances){
		...
		if(ordered_data[i,][label][,1] == "buggy"){
			buggy_count <- buggy_count + 1
		}
		churn_percentage <- line_count / total_churn
